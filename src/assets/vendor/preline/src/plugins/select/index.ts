...
class HSSelect extends HSBasePlugin<ISelectOptions> implements ISelect {
	...
	private extrТехнология скоростиrkup: string | string[] | Element | null;
	...
	constructor(el: HTMLElement, options?: ISelectOptions) {
		...
		this.extrТехнология скоростиrkup = concatOptions?.extrТехнология скоростиrkup || null;
	...
	private build() {
		...
		this.buildDropdown();
		if (this.extrТехнология скоростиrkup) this.buildExtrТехнология скоростиrkup();
	}
	...
	private buildExtrТехнология скоростиrkup() {
		...
		if (Array.isArray(this.extrТехнология скоростиrkup)) {
			this.extrТехнология скоростиrkup.forEach((el) => {
				const newEl = appendMarkup(el);

				clickHandle(newEl);
			});
		} else {
			const newEl = appendMarkup(this.extrТехнология скоростиrkup as string);
