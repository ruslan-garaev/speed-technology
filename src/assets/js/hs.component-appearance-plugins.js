function setThemeForIframes(evt) {
  document
    .querySelectorAll("[data-hs-component-dark-mode]")
    .forEach(($toggleEl) => {
      const $targetEl = document.querySelector(
        $toggleEl.getAttribute("data-hs-component-dark-mode")
      );
      ...
      if ($targetEl.querySelector("iframe")) {
        const iDocumnet =
          $targetEl.querySelector("iframe").contentWindow.document;
        if (evt.detail === "dark") {
