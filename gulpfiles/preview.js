...
function fileInclude() {
  ...
  return gulp
    ...
    .pipe(replace(new RegExp('(\\.+\\/)+' + "node_modules" + '\/.*\\.*', 'g'), function (match, p1) {
      ...
      return match.replace('node_modules', paths.github.vendor.dir.replace(paths.github.base.dir + '/', ''))
    }))
    ...
    .pipe(replace(/\@\@node_modules/g, function (math, p1) {
      return 'node_modules'
    }))
    ...
    .pipe(replace(/<!-- bundlejs:theme \[(.*?)\](.*)-->/g, function (math, p1, p2) {
      return `<script src="${p1}/${paths.github.build.js}/${config.fileNames.dist.js}${p2.trim()}"></script>`;
    }))
    ...
    .pipe(gulp.dest(paths.github.base.dir))
};
...
function minCSS() {
  return gulp
    ...
    .pipe(gulp.dest(paths.github.css.dir))
}
...
function minJS() {
  ...
  if (bundle.length) {
    return gulp
      ...
      .pipe(uglify())
      .pipe(gulp.dest(paths.github.js.dir));
  } else {
...
function copy() {
  return gulp
    ...
    .pipe(gulp.dest(paths.github.assets.dir))
}
...
function copyNode() {
  ...
  if ([...node].length) {
    return gulp
      .src([...node])
      .pipe(gulp.dest(paths.github.vendor.dir));
  } else {
    return new Promise(function (resolve, reject) {
      resolve();
    });
...
function copyDependencies() {
  for (var k in config.copyDependencies.dist) {
    ...
    gulp
      .src(path)
      .pipe(gulp.dest(paths.github.base.dir + '/' + config.copyDependencies.dist[k]))
  }
...
function clean() {
  return del(paths.github.base.dir, {force: true});
}

gulp.task('github', gulp.series(clean, fileInclude, CSS, minCSS, minJS, svgCompiler, copy, copyNode, copySkipedFiles, copyDependencies, copyFavicon));
